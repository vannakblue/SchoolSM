...
def run_import():
    ...
    if not os.path.exists(xlsm_path):
        print(f"Error: File not found at {xlsm_path}")
        return

    print("==================================================================")
    print("STEP 1: Setting up Academic Year 2026-2027 as Active")
    print("==================================================================")
    ...
    ay.is_current = True
    ay.save()
    print(f"Academic Year: {ay.name} (Active={ay.is_current})")

    print("\n==================================================================")
    print("STEP 2: Ensuring exactly 40 standard classrooms for 2026-2027")
    print("==================================================================")
    official_40 = [
    ...
    with transaction.atomic():
        for code, grade, track in official_40:
            ...
            # Assign default standard subjects
        ...
        # Delete any stray/redundant classrooms for 2026-2027
        redundant = Classroom.objects.filter(academic_year=ay).exclude(code__in=valid_codes)
        for rc in redundant:
            rc.students.all().delete()
            rc.delete()

    print(f"Verified {len(classroom_map)} classrooms in 2026-2027.")

    print("\n==================================================================")
    print("STEP 3: Wiping old student data cleanly")
    print("==================================================================")
    old_student_count = Student.objects.count()
    print(f"Deleting {old_student_count} existing students and linked records...")
    ...
    print(f"Clean wipe complete. Remaining students: {Student.objects.count()}")

    print("\n==================================================================")
    print("STEP 4: Importing students from E:\\SchoolSM\\2026-2027.xlsm")
    print("==================================================================")
    ...
    for sname in ['7', '8', '9', '10', '11', '12']:
        ...
        for r_idx, r in enumerate(rows[1:], start=2):
            ...
            if not target_class:
                print(f"Warning: Classroom '{class_code}' not found for row {r_idx} in Sheet {sname}")
                continue

            # Ensure ID uniqueness (handle collisions)
            clean_id = st_id
        ...
        sheet_counts[sname] = sheet_created
        print(f"  - Sheet {sname:2s}: {sheet_created} students prepared")

    with transaction.atomic():
        Student.objects.bulk_create(to_create, batch_size=500)

    print(f"\n==================================================================")
    print(f"IMPORT COMPLETE: {Student.objects.count()} students inserted successfully!")
    print(f"==================================================================")

    # Verification per grade and classroom
    print("\nSUMMARY BY GRADE AND CLASS:")
    for grade in [7, 8, 9, 10, 11, 12]:
        classes = Classroom.objects.filter(academic_year=ay, grade_level=grade).order_by('code')
        total_g = Student.objects.filter(classroom__in=classes, status='ACTIVE').count()
        fem_g = Student.objects.filter(classroom__in=classes, status='ACTIVE', gender='F').count()
        details = []
        for c in classes:
            c_cnt = c.students.filter(status='ACTIVE').count()
            c_fem = c.students.filter(status='ACTIVE', gender='F').count()
            details.append(f"{c.code}:{c_cnt}(F:{c_fem})")
        print(f"Grade {grade:2d} (Total: {total_g:3d}, F: {fem_g:3d}): " + " | ".join(details))
