...
class ToolAiService:
    ...
    def _build_prompt(cls, tool_name, action, content, options, school_name, year_name, user_role, user_name):
        ...
        # -------------------------------------------------------------
        # 5. Khmer Lunar Date Converter Actions
        # -------------------------------------------------------------
        elif tool_name == 'khmer_lunar_converter':
            prompt = (
                f"អ្នកប្រើប្រាស់កំពុងពិនិត្យមើលកាលបរិច្ឆេទចន្ទគតិខ្មែរ៖ *«{content}»*。\n\n"
                f"ក្នុងនាមជាអ្នកជំនាញប្រវត្តិសាស្ត្រ ប្រតិទិនចន្ទគតិខ្មែរ និងវប្បធម៌ប្រពៃណីខ្មែរ សូម៖\n"
                f"1. 🌕 **ពន្យល់អំពីអត្ថន័យនៃថ្ងៃចន្ទគតិនេះ** (ថ្ងៃកើត/រោច ខែ ឆ្នាំសត្វ ស័ក និងពុទ្ធសករាជ)\n"
                f"2. ☸️ **បញ្ជាក់ថាតើជាថ្ងៃសីល ឬមានពិធីបុណ្យជាតិ/សាសនាអ្វីដែលត្រូវប្រារព្ធ** (បើមាន)\n"
                f"3. 📜 **ទំនៀមទម្លាប់ ប្រពៃណី ឬដំបូន្មានល្អៗសម្រាប់ការប្រកបកិច្ចការងារ ប្រឡង ឬកម្មវិធីសាលារៀន**"
            )

        # -------------------------------------------------------------
        # 6. Tools Hub Smart Finder
        # -------------------------------------------------------------
        elif tool_name == 'hub':
            prompt = (
    ...
    def _get_smart_fallback(cls, tool_name, action, content, options, school_name):
        if tool_name == 'text_analyzer':
            ...
            return (
                f"🌕 **ព័ត៌មានកាលបរិច្ឆេទចន្ទគតិខ្មែរ (Khmer Lunar Guide)**:\n\n"
                f"• **កាលបរិច្ឆេទ:** {content.strip()}\n"
                f"• **ប្រភព:** ប្រតិទិនចន្ទគតិខ្មែរផ្លូវការ គណនាតាមក្បួនហោរាសាស្ត្រ និងតារាសាស្ត្របុរាណខ្មែរ ស្របតាមស្តង់ដារ MoEYS។\n"
                f"• **ការអនុវត្ត:** ប្រសិនបើត្រូវនឹងថ្ងៃសីល (៨កើត, ១៥កើត, ៨រោច, ដាច់ខែ) ពុទ្ធសាសនិកខ្មែរតែងតែចូលរួមរក្សាសីល ធ្វើបុណ្យ និងស្តាប់ព្រះធម៌។\n\n"
                f"💡 *កំណត់សម្គាល់:* ភ្ជាប់ជាមួយ Gemini API ដើម្បីទទួលបានការពន្យល់លម្អិតបន្ថែមពី Gemini 3.8 Flash។"
            )
